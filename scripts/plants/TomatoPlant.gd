...
# Override the _ready function to set Tomato-specific properties
func _ready():
	...
	if not seed_mesh:
		push_error("TomatoPlant: SeedMesh node not found!")
	if not growing_mesh:
		push_error("TomatoPlant: GrowingMesh node not found!")
	if not leaves:
		push_error("TomatoPlant: Leaves node not found!")
...
func update_appearance():
	if not seed_mesh or not growing_mesh or not leaves:
		push_error("TomatoPlant.update_appearance: Missing node references!")
		return
